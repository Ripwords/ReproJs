...
/* === Gallery browse sheet (Details step -> "Choose from gallery…") === */
.ft-sheet-backdrop {
  position: absolute;
  inset: 0;
  z-index: 2;
  ...
  justify-content: center;
  padding: 16px;
  background: var(--ft-color-scrim, rgba(0, 0, 0, 0.45));
}
.ft-sheet {
  display: flex;
  flex-direction: column;
  ...
  border: 1px solid var(--ft-color-border);
  border-radius: var(--ft-radius-lg);
  box-shadow: var(--ft-shadow-lg, 0 18px 48px rgba(0, 0, 0, 0.3));
}
.ft-sheet-head {
  padding: 14px 16px;
  font-size: 13px;
