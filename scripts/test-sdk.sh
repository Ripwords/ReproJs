...
for pkg in "${PACKAGES[@]}"; do
  ...
  #
  # Run from the PACKAGE directory, not the repo root: bun resolves
  # tsconfig.json from the cwd, and there is no root tsconfig — from the
  # repo root, JSX files silently transpile against the default
  # `jsxImportSource: react` instead of the package's `preact`, and the
  # first test whose import graph reaches real JSX syntax dies with
  # "Cannot find module 'react/jsx-dev-runtime'". Package cwd matches how
  # developers run these suites locally (`cd packages/<pkg> && bun test`).
